# fxtwitter-bot

指定した Discord チャンネルに投稿された Twitter / X のURLを、[FxEmbed](https://github.com/FixTweet/FxTwitter) のURL（`fxtwitter.com`）へ自動的に置き換える小さな Bot です。

Discord の標準の埋め込みでは動画がインラインで再生できないことがありますが、`fxtwitter.com` に置き換えると動画が再生・保存しやすくなります。

さらに、チャンネルに溜まった共有履歴からランキングを集計する機能を備えています。`/ranking` コマンドでいつでも呼び出せるほか、週次・月次で自動投稿します。

## URLの置き換え

対象チャンネルに Twitter / X のURLを含むメッセージが投稿されると、Bot は次の処理を行います。

1. 元のメッセージを削除する
2. URLを `https://fxtwitter.com/...` に置き換え、`**投稿者名**: 本文` の形式で投稿し直す

```
入力:  https://x.com/example/status/1234567890
出力:  **someone**: https://fxtwitter.com/example/status/1234567890
```

1つのメッセージに複数のURLが含まれる場合、すべて置き換えられます。Bot 自身の投稿は無視されます。

> **注意**: 「削除して投稿し直す」方式のため、元メッセージの返信関係・添付ファイル・リアクションは失われます。

## ランキング (`/ranking`)

チャンネルで `/ranking` を実行すると、共有された動画ツイートを集計して投稿します。期間は **直近7日間 / 直近30日間 / 全期間** から選べます（既定は直近7日間）。

```
# 🏆 直近30日間 のランキング

## 📊 全体
投稿 55件 ／ 参加 4人 ／ 前月比 +37件

## 👑 投稿数ランキング
🥇 someone — 37件
🥈 anyone — 14件
🥉 everyone — 2件

## 💥 最強ツイート
@cat_movie — ❤️ 35.3万
> かわいい猫です

## 🐦 よく貼られたアカウント
- @cat_movie — 4件

## 🔤 よく出たハッシュタグ
#猫 (3) 　#犬 (1)
```

### 定期投稿

手動実行に加えて、次のタイミングで自動的にランキングを投稿します（日本時間）。

| 種類 | 投稿時刻 | 集計期間 |
| --- | --- | --- |
| 週次 | 毎週月曜 9:00 | 先週の月曜〜日曜 |
| 月次 | 毎月1日 9:00 | 先月まるごと |

`/ranking` が「直近7日間」のような現在からの相対期間で集計するのに対し、定期投稿は暦どおりの週・月を対象にします。

投稿済みかどうかは、本文の末尾に添えた識別子（`-# 週次 2026-W36` など）をチャンネル履歴から探して判定します。プロセスの記憶に頼らないため、**再起動やデプロイをまたいでも二重投稿しません**。また Bot が停止していた期間があれば、復帰後に遅れて投稿されます。

### AI による総括（任意）

`ANTHROPIC_API_KEY` を設定すると、[Claude API](https://docs.anthropic.com/) を使って「その期間はどんな話題が多かったか」の総括を生成し、ランキングの冒頭に添えます。**この機能は任意です。** キーが未設定なら無効のまま起動し、Bot の他の機能は問題なく動作します。

設計方針は以下のとおりです。

- **定期投稿のときだけ生成します。** `/ranking` の手動実行では生成しません。実行回数に応じて課金が読めなくなるのを避けるためです
- **送信するのはツイート本文のみです。** Discord の表示名やチャンネルの情報は送信しません
- **生成に失敗しても、ランキング本体は必ず投稿されます**（総括の行が省略されるだけ）
- 送信件数（200件）と1件あたりの文字数（300文字）に上限を設けています
- ツイート本文は「指示ではなくデータ」としてプロンプト内で明示し、本文中の文言に従わないようにしています

モデルは `claude-opus-5`、思考の深さは `low` を指定しています。集計対象が50件程度であれば、1回あたり10〜15円が目安です。

### データベースを持たない設計

集計に**データベースを使いません**。Bot が投稿し直したメッセージそのものを台帳として扱い、実行時にチャンネル履歴を遡って数えます。

この方式には次の利点があります。

- 保存先の用意も費用も不要で、再デプロイしてもデータが消えない
- **機能を追加した時点で、過去の履歴すべてを遡って集計できる**

一方で、次の制約があります。

- 投稿者は Discord の**表示名**で識別するため、改名すると別人として数えられます
- 削除されたメッセージは集計対象から外れます
- Bot の投稿フォーマット（`**投稿者名**: 本文`）が実質的なデータ形式になるため、**変更すると過去の履歴を読めなくなります**

なお Bot 自身のランキング投稿を数えないよう、集計対象は「`**投稿者名**: ` で始まる投稿」に限定しています。

### FxTwitter API の利用について

「最強ツイート」「ハッシュタグ」および元アカウントの正確な特定には、[FxTwitter](https://github.com/FixTweet/FxTwitter) の公開 API (`api.fxtwitter.com`) を利用します。有志が無償で運営しているサービスのため、次の配慮を実装しています。

- 同時接続は4本まで、1回の集計につき最大300件
- 取得済みの結果はプロセス内にキャッシュし、再問い合わせしない
- 10秒でタイムアウト
- **API を利用できなくても集計は成立します**（該当セクションが省略されるだけ）

`https://x.com/i/status/...` のようにURLへユーザー名が含まれない形式でも、この API により本来の投稿者を特定できます。

## 同種の Bot との違い

同じ目的の Bot は多数存在します（[FixTweetBot](https://github.com/Kyrela/FixTweetBot) など）。このプロジェクトの位置づけは以下のとおりです。

- **元メッセージを削除して置き換える**方式。返信で追記する Bot と違い、チャンネルにリンクが二重に並びません
- **共有履歴からランキングを集計できる**。しかもデータベースを必要としません
- **自分でホストする前提**。他人が運用する Bot にメッセージの読み取り・削除権限を渡さずに済みます
- Rust 製の最小構成で、Fly.io の無料枠（shared-cpu-1x / 256MB）で動きます

多機能さを求める場合は、既存の公開 Bot のほうが適しています。

## 必要なもの

- Rust（edition 2021。Docker / CI では 1.98 を使用）
- Discord Bot アカウント

### Discord 側の設定

[Discord Developer Portal](https://discord.com/developers/applications) で Bot を作成し、以下を設定してください。

**特権インテント**（Bot → Privileged Gateway Intents）

- `MESSAGE CONTENT INTENT` を **有効化**（メッセージ本文を読むために必須）

**OAuth2 スコープ**（OAuth2 → URL Generator でサーバーへ招待する際に指定）

| スコープ | 用途 |
| --- | --- |
| `bot` | Bot 本体 |
| `applications.commands` | `/ranking` の登録 |

**Bot に必要な権限**

| 権限 | 用途 |
| --- | --- |
| メッセージを送信 (Send Messages) | 置換後のメッセージの投稿 |
| メッセージの管理 (Manage Messages) | 元メッセージの削除 |
| メッセージ履歴を読む (Read Message History) | ランキングの集計 |

「メッセージの管理」がないと削除に失敗し、置換後のメッセージも投稿されません。
「メッセージ履歴を読む」がないと `/ranking` が集計できません。

`/ranking` は起動時にサーバー単位で登録されます（サーバー単位の登録は即座に反映されます）。既に導入済みの Bot にランキング機能を追加する場合は、`applications.commands` を含めた招待URLで**再度承認**してください。サーバーから退出させる必要はありません。

## 設定

すべて環境変数で指定します。いずれか一方でも欠けていると、Bot は起動時にエラーで停止します。

| 環境変数 | 必須 | 説明 |
| --- | --- | --- |
| `DISCORD_TOKEN` | ○ | Discord Bot のトークン |
| `ALLOWED_CHANNEL_IDS` | ○ | 対象チャンネルのID。カンマ区切りで複数指定可 |
| `ANTHROPIC_API_KEY` | | 設定するとAI総括が有効になる。未設定でも Bot は動作します |

`ALLOWED_CHANNEL_IDS` の例:

```sh
ALLOWED_CHANNEL_IDS="123456789012345678"                       # 1チャンネル
ALLOWED_CHANNEL_IDS="123456789012345678,987654321098765432"    # 複数チャンネル
```

チャンネルIDは、Discord の設定で開発者モードを有効にすると、チャンネルを右クリックしてコピーできます。

## ローカルでの実行

```sh
export DISCORD_TOKEN="あなたのトークン"
export ALLOWED_CHANNEL_IDS="対象チャンネルのID"
cargo run --release
```

## テスト

```sh
cargo test
```

集計・整形まわりのロジックは Discord に接続せずテストできるようになっています。

FxTwitter API へ実際に接続して応答の形を確かめるテストは、外部サービスに依存するため通常の実行からは除外しています。必要なときだけ次で実行してください。

```sh
cargo test -- --ignored
```

## Docker

```sh
docker build -t fxtwitter-bot .
docker run \
  -e DISCORD_TOKEN="あなたのトークン" \
  -e ALLOWED_CHANNEL_IDS="対象チャンネルのID" \
  fxtwitter-bot
```

ビルドは2段構成です。`rust:1.98-bookworm` でコンパイルし、実行ファイルだけを `debian:bookworm-slim` へコピーします。実行側と同じ `bookworm` を明示しているのは、glibc のバージョンを揃えて実行時エラーを避けるためです。バージョンを変更する際は、両者の Debian を揃えてください。

## Fly.io へのデプロイ

`fly.toml` は東京リージョン（`nrt`）の shared-cpu-1x / 256MB で動く構成です。HTTP を待ち受けないワーカーとして常時稼働します。

```sh
fly launch --no-deploy          # 初回のみ（アプリ名は適宜変更してください）
fly secrets set DISCORD_TOKEN="あなたのトークン"
fly secrets set ALLOWED_CHANNEL_IDS="対象チャンネルのID"
fly deploy
```

`ALLOWED_CHANNEL_IDS` は機密情報ではありませんが、環境ごとに異なる値なので `fly secrets` で渡し、リポジトリには含めていません。

### GitHub Actions による自動デプロイ

`main` ブランチへの push で `.github/workflows/fly-deploy.yml` が実行され、Fly.io へデプロイされます。

リポジトリの **Settings → Secrets and variables → Actions** に `FLY_API_TOKEN` を登録してください。アカウント全体のトークンではなく、アプリに限定したデプロイトークンを推奨します。

```sh
fly tokens create deploy -x 8760h    # このアプリのデプロイのみ可能・有効期限1年
```

デプロイに使う action はコミットSHAで固定しています。中身が予告なく差し替わることを防ぐためです。更新する場合は、対象リポジトリのタグに対応するSHAへ書き換えてください。

## ライセンス

[MIT](LICENSE)
